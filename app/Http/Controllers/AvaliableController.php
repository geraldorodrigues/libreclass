...
class AvaliableController extends Controller
{
	...
	public function postSave()
	{
		if (Input::has("exam")) {
			...
			$exam = new Exam;
			$exam->unit_id = Crypt::decrypt(Input::get("unit"));
			$exam->aval = "A";
		}
	...
	public function getNew()
	{
		...
		//		$frequency->id_attend = $attend->id;
		//		$frequency->idLesson = $lesson->id;
		//		$frequency->value = "P";
		//		$frequency->save();
		//	}
		return view("modules.avaliable", ["user" => auth()->user(), "exam" => $exam, "students" => [], "unit" => $unit]);
//	return Redirect::to("/avaliable?e=" . Crypt::encrypt($exam->id))->with("message", "Click em salvar para criar .");;
	...
	public function postExam()
	{
		try {
			...
			if (ExamsValue::where("id_attend", $attend)->where("exam_id", $exam)->first()) {
				ExamsValue::where("id_attend", $attend)->where("exam_id", $exam)->update(["value" => $value]);
			} else {
