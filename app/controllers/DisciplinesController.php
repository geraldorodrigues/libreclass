...
//    $discipline->course = Crypt::encrypt(Course::find(Period::find($discipline->idPeriod)->id)->id);
    ...
    {
        if (Input::get("course")) {
            $disciplines = DB::select("SELECT Disciplines.id AS id, Disciplines.name AS name, Periods.name AS period FROM Disciplines, Periods WHERE idPeriod = Periods.id AND idCourse = ? AND Disciplines.status = 'E' ORDER BY Periods.name ASC, Disciplines.name ASC", [Crypt::decrypt(Input::get("course"))]);
